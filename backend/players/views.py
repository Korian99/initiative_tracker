...
def player_connect(request):
    player_name = request.POST.get("player")
    player, _ = Player.objects.get_or_create(name=player_name)


    lobbies_id = PlayerInLobby.objects.filter(player=player).values_list("lobby_id",flat=True)
...
def add_character(request):
    player_in_lobby = get_object_or_404(PlayerInLobby, id=request.POST.get("player"))
    ...
    if Character.objects.filter(name=character_name, player=player_in_lobby).exists():
        existing_characters = Character.objects.filter(name=character_name, player=player_in_lobby)
        ...
        Character.objects.create(player=player_in_lobby, name=character_name, initiative=initiative)
...
def character_list_partial(request, lobby_code):
